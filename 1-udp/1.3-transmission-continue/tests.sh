...
test_4()
{
    ...
    nettoyer
    local pid
    generer_fichier_aleatoire $TMP.in 16
    $PROGR $PORT > $TMP.out 2> $TMP.err &
    pid=$!
    sleep 2
    $PROGS $IP4 $PORT < $TMP.in > $TMP.out2 2> $TMP.err2
    sleep 1
    kill $pid && wait $pid
    est_vide $TMP.err2 || fail "$PROGS la sortie d'erreur doit être vide"
    est_vide $TMP.err  || fail "$PROGR la sortie d'erreur doit être vide"
    est_vide $TMP.out && fail "aucune donnée reçue"
    cmp $TMP.in $TMP.out > $TMP.diff || fail "les données reçues sont différentes des données envoyées"
    nettoyer
    echo OK
}
...
test_5()
{
    ...
    pid=$!
    sleep 2
    $PROGS $IP6 $PORT < $TMP.in > $TMP.out2 2> $TMP.err2
    sleep 1
    kill $pid && wait $pid
    est_vide $TMP.err2 || fail "$PROGS la sortie d'erreur doit être vide"
    est_vide $TMP.err  || fail "$PROGR la sortie d'erreur doit être vide"
